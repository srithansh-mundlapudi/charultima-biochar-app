...
const FAQ = () => {
  ...
  const faqData = [
    {
      ...
      answer:
        'CharUltima was designed as a Progressive Web Application to provide farmers with easy access to the platform, whether through a website or an app. This approach ensures that farmers can use the application anytime, anywhere, making it more convenient and versatile.',
    },
    {
      id: 2,
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-8">
      {' '}
      {/* Increased space-y-8 to space out questions more */}
      <h1 className="text-4xl font-bold text-center mb-8 text-gray-800">
        Frequently Asked Questions
      </h1>
      {faqData.map((item) => (
        <div key={item.id} className="border border-gray-300 rounded-lg overflow-hidden shadow-sm">
          <button
          ...
          >
            ...
            <span className="text-3xl">{expandedItems[item.id] ? '↑' : '↓'}</span>{' '}
            {/* Larger arrows */}
          </button>

          {expandedItems[item.id] && (
            <div className="p-6 bg-gray-50 text-gray-700">{item.answer}</div>
          )}
        </div>
      ))}
    </div>
